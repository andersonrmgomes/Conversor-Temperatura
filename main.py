...
label_titulo = ctk.CTkLabel(frame_esquerdo, text="Conversor de Temperatura", font=("Arial", 20), anchor="center")
label_titulo.pack(pady=10, padx=20, anchor="center")
largura_input = 220
entrada_temp = ctk.CTkEntry(frame_esquerdo, placeholder_text="Digite a temperatura", width=largura_input)
resultado = StringVar(value="Resultado")
menu_opcoes = ctk.CTkComboBox(frame_esquerdo, values=["Celsius para Fahrenheit", "Fahrenheit para Celsius"], state="readonly", width=largura_input)

# Define uma opção padrão no ComboBox
menu_opcoes.set("Celsius para Fahrenheit")

entrada_temp.pack(pady=10, padx=20, anchor="w")
menu_opcoes.pack(pady=5, padx=20, anchor="w")
ctk.CTkButton(frame_esquerdo, text="Converter", command=converter_temperatura).pack(pady=20, padx=20, anchor="w")

botao_tema = ctk.CTkButton(frame_esquerdo, text="⚙", command=alternar_tema, width=40, height=40, font=("Segoe UI Symbol", 20))
botao_tema.place(x=10, y=345)

label_resultado = ctk.CTkLabel(frame_direito, textvariable=resultado, font=("Arial", 45), anchor="center", width=300, height=200)
